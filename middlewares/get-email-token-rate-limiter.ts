  ...
  async handler(
    ...
    try {
      ...
      if (request.url.pathname.includes('verify')) {
        ...
        await this.rateLimiterLow.consume(
          request.remoteAddress.ip ?? 'no-ip',
          1,
          {},
        )
        await this.rateLimiterHight.consume(
          request.remoteAddress.ip ?? 'no-ip',
          1,
          {},
        )
      ...
      if (e instanceof RateLimiterRes) {
        ...
        response.headers.set(
          'Retry-After',
          Math.ceil(e.msBeforeNext / 1000).toString(),
        )
        response.headers.set('X-RateLimit-Limit', '5')
        response.headers.set(
          'X-RateLimit-Remaining',
          e.remainingPoints.toString(),
        )
        response.headers.set(
          'X-RateLimit-Reset',
          (e.msBeforeNext / 1000).toFixed(0).toString(),
        )
      } else {
